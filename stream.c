...
main(int argc, char* argv[])
	...
	int opt = 0;
	while ((opt = getopt(argc, argv, "hn:s:o:")) != -1) {
		switch (opt) {
			case 'n':
				NTIMES = atoi(optarg);
				break;
			case 's':
				STREAM_ARRAY_SIZE = strtoull(optarg, NULL, 10);
				break;
			case 'o':
	...
	STREAM_TYPE* a = malloc(sizeof(STREAM_TYPE) * (STREAM_ARRAY_SIZE+OFFSET));
	STREAM_TYPE* b = malloc(sizeof(STREAM_TYPE) * (STREAM_ARRAY_SIZE+OFFSET));
	STREAM_TYPE* c = malloc(sizeof(STREAM_TYPE) * (STREAM_ARRAY_SIZE+OFFSET));

	double* times[4];
	for (int i = 0; i < 4; ++i)
		times[i] = malloc(sizeof(double) * NTIMES);
